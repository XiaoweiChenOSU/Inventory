...
// dd($data);
	...
	public function product_code(Request $request){
		...
		$queries = \App\Models\Product::where('code', 'LIKE', '%'.$term.'%')->take(5)->get();
	...
	public function product_sku(Request $request){

		$term = $request['term'];
		
		$results = array();
		
		$queries = \App\Models\Product::where('sku', 'LIKE', '%'.$term.'%')->take(5)->get();

		foreach ($queries as $key => $value)
		{
			$queries[$key]['value'] = $value->sku;
		}
